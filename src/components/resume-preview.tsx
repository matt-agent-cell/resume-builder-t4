...
export default function ResumePreview() {
  ...
  return (
    <div className="h-full flex flex-col bg-stone-100 relative">
      ...
      {activeTab === "resume" && (
      <div className="bg-white max-w-[8.5in] mx-auto shadow-xl rounded-lg px-8 py-7" style={{ fontFamily, color: textColor, fontSize: `${basePx}px`, lineHeight, minHeight: "11in" }}>
        {/* Header */}
        <div style={{ marginBottom: sectionGap, paddingBottom: 16, borderBottom: borderStyle === "none" ? "none" : `2px ${borderStyle} ${headingColor}` }}>
                ...
                <ul className="mt-1 space-y-0.5 ml-3">
                  {exp.bullets.map((bullet, bi) => (
                    <li key={bi} className="flex gap-1.5">
                      <span style={{ color: `${textColor}66` }} className="mt-px">•</span>
                      <InlineEdit
                        value={bullet}
                        onSave={(v) => {
                          updateResume((r) => ({
          ...
          <div className={hl("skills")}>
            ...
            <div className="flex flex-wrap gap-1.5">
              {resume.skills.map((skill, i) => (
                <span key={i} style={{ backgroundColor: `${accentColor}15`, color: accentColor, fontSize: `${basePx * 0.83}px` }} className="font-medium px-2.5 py-1 rounded-full">{skill}</span>
              ))}
            </div>
          </div>
        )}
